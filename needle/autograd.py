...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
      ...
      if node.op is None:
        continue
      # compute partial ajoints for input node use upstream grad
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    ### BEGIN YOUR SOLUTION
    visited = set() # 不同路径可能先后访问到同一个节点，需要标记已访问节点
    topo_order = [] # 往里面添加节点
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for in_node in node.inputs:
      topo_sort_dfs(in_node, visited, topo_order) # 递归，直到 node.inputs=[]
